    ...
    def parse_data_points(self, weather, air_quality, tz, units):
        ...
        if sunrise_epoch:
            ...
            data_points.append({
                ...
                "unit": '',  # Use sunrise_dt.strftime('%p') for AM
        ...
        if sunset_epoch:
            ...
            data_points.append({
                ...
                "unit": '',  # Use sunrise_dt.strftime('%p') for PM
        ...
        data_points.append({
            "label": "Wind",
            "measurement": round(weather.get('current', {}).get("wind_speed", 0)),
        ...
        data_points.append({
            "label": "Pressure",
            "measurement": round(weather.get('current', {}).get("pressure", 0) * 0.02953, 2),
            "unit": 'in',
        ...
        visibility = weather.get('current', {}).get("visibility")/ 1609.344
        #visibility_str = str(round(visibility, 2))
        visibility_str = f">{int(round(visibility, 0))}" if visibility >= 6.2 else str(int(round(visibility, 0)))
        data_points.append({
            "label": "Visibility",
